...
abstract class CrudServiceImpl<T : IdentifierEntity?, ID> : CrudService<T, ID> {
    ...
    override fun findAll(): Iterable<T> = getRepository().findAll().sortedBy { entity -> entity?.id }
    override fun findAllById(ids: Iterable<ID>): Iterable<T> = getRepository().findAllById(ids).sortedBy { entity -> entity?.id }
